...
static ssize_t
udplite_trans_rw(int file_fd, int connected_fd)
{
	int buflen;
...
static ssize_t
udplite_trans_mmap(int file_fd, int connected_fd)
{
	int ret = 0;
...
static ssize_t
udplite_trans_splice(int file_fd, int connected_fd)
{
#ifdef HAVE_SPLICE
	int pipefds[2];
	struct stat stat_buf;
...
static ssize_t
udplite_trans_sendfile(int file_fd, int connected_fd)
{
	struct stat stat_buf;
	ssize_t rc, write_cnt;
	off_t offset = 0;
...
udplite_trans_mode(void)
{
	...
	switch (opts.io_call) {
	case IO_SENDFILE:
		udplite_trans_sendfile(file_fd, connected_fd);
		break;
	case IO_SPLICE:
		udplite_trans_splice(file_fd, connected_fd);
		break;
	case IO_MMAP:
		udplite_trans_mmap(file_fd, connected_fd);
		break;
	case IO_RW:
		udplite_trans_rw(file_fd, connected_fd);
		break;
	default:
		err_msg_die(EXIT_FAILINT, "Programmed Failure");
	}
