...
=item B<tcp>, B<udp>, B<udplite>, B<dccp>, B<sctp>, B<unix> or B<tipc>.

=back

When using the unix or tipc address family, you must also specify a socket type, e.g. B<netsend tipc MODE SOCK_STREAM>.
...
=item B<-u>
	...
	by the network protocol used (in this case, you may split data using the -b option on the sender side).
...
B<Listen for incoming SCTP connections, incoming data goes to stdout:>
...
B<Send file largefile via TCP with output in machine parseable format:>
...
B<Receive data via TCP with MD5SIG from peer 10.0.0.1:>
...
B<Transmit using TCP with MD5SIG to 10.0.0.1 with hybla congestion control:>

netsend -s TCP_CONGESTION hybla tcp transmit -C largefile \ ffff::10.0.0.1

=back

=over 1

B<Receive and transmit a file using TIPC seqpacket socket:>

netsend tipc receive SOCK_SEQPACKET /tmp/recvFile

netsend tipc transmit SOCK_SEQPACKET /path/to/sendFile
...
=head1 BUGS
  ...
  please report bugs to <fw-bugs-netsend@strlen.de>.
