...
gen_human_analyse(char *buf, unsigned int max_buf_len)
{
	...
	/* real time */
	len += xsnprintf(buf + len, max_buf_len - len, "%s %.4f sec\n",
	...
	len += xsnprintf(buf + len, max_buf_len - len, "%s %.4f sec (cpu/real: %.2f%%)\n",
			T2S(STAT_CPU_TIME), total_cpu, (total_cpu / total_real ) * 100);
#ifdef HAVE_RDTSCLL
	len += xsnprintf(buf + len, max_buf_len - len, "%s %lld cycles\n",
			T2S(STAT_CPU_CYCLES),
	...
	if (opts.verbose >= LOUDISH) {
		...
		/* swaps */
		len += xsnprintf(buf + len, max_buf_len - len, "%s %ld \n",
				T2S(STAT_SWAPS), res);
		...
		len += xsnprintf(buf + len, max_buf_len - len, "%s %ld bytes\n",
...
gen_mashine_analyse(char *buf, unsigned int max_buf_len)
{
	...
	** clean (\d+))
	*/
	len += xsnprintf(buf + len, max_buf_len - len, "%d ",
			opts.buffer_size ?
			opts.buffer_size : 0);
	...
	*/
	...
	if (total_real <= 0.0)
		total_real = 0.00001;
	len += xsnprintf(buf + len, max_buf_len - len, "%.4f ", total_real);

	/* 7. user time */
	subtime(&net_stat.use_stat_end.ru.ru_utime, &net_stat.use_stat_start.ru.ru_utime, &tv_tmp);
	total_utime = tv_tmp.tv_sec + ((double) tv_tmp.tv_usec) / 1000000;
	len += xsnprintf(buf + len, max_buf_len - len, "%.4f ", total_utime);

	/* 8. system time */
	subtime(&net_stat.use_stat_end.ru.ru_stime, &net_stat.use_stat_start.ru.ru_stime, &tv_tmp);
	total_stime = tv_tmp.tv_sec + ((double) tv_tmp.tv_usec) / 1000000;
	len += xsnprintf(buf + len, max_buf_len - len, "%.4f ", total_stime);

	/* 9. cpu-time */
	total_cpu = total_utime + total_stime;
	if (total_cpu <= 0.0)
		total_cpu = 0.0001;
	len += xsnprintf(buf + len, max_buf_len - len, "%.4f ", total_cpu);
	...
	len += xsnprintf(buf + len, max_buf_len - len, "%ld ", res);
